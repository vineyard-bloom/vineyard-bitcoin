...
Utils.findTxsByAddress = function (txps, id) {
    var matches = _.filter(txps, function (tx) {
        //console.log('tx: ', tx.outputs[0].address, ", ", id);
        return _.contains(tx.outputs[0].address, id);
    });
    if (!matches.length)
        Utils.die('Could not find TXs for:' + id);
    return matches;
};
Utils.UNITS = {
    'btc': 100000000,
